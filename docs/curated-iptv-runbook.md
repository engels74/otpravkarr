...
The deployed Otpravkarr image was built and published by GitHub Actions from reviewed runtime source commit `06d91dd3df567a08257380c039df7d4eb5d9e263`; Compose pins `ghcr.io/engels74/otpravkarr-docker@sha256:a36dcfc3abe7b0b6be873ac5a6516287d565e627d9c86a75857a668d132d3ac1`. Remote update run `30441836873` and multi-architecture build run `30441894432` passed. Later commits on `main` update only reviewed evidence and operator artifacts, not runtime source. A bounded corrupt-snapshot injection proved membership remains unchanged, the failure is logged, and recovery succeeds after restoring valid state. Pullio updates are paused for Dispatcharr and Otpravkarr. The latest final-rollout receipt identifies a coherent SQLite/Compose backup, hashes both copies, and records matching source/restored integrity checks; protected phase backups also retain prior image IDs/digests, Caddy copies, PostgreSQL data, plugin settings, and checkpoints. A later degraded public health signal was traced to a stale Plex resource endpoint; after a fresh restore-tested backup, the supported authenticated settings action selected a currently reachable local, non-relay Plex resource. Two public health reads returned `ok`, and Docker health plus SQLite integrity remained healthy.
