...
- **Auth.** Single cookie `otpravkarr_session` holds either an admin or user session row (`sessions.session_type`). Use `requireAdmin(event)` / `requireAdminApi(event)` / `requireUser(event)` from `$lib/server/auth.ts`. Admin sessions live 1h; user sessions 4h. `requireAdmin*` redirect (303 → `/login`) or error 401 for API. `sessionResolver` only places **active** user mappings in `locals.user`; an inactive (revoked) mapping goes to `locals.revokedUser` instead (and `locals.user` is null), so portal chrome can still render the revoked view + "Inactive" badge while credential-serving sinks never see a revoked user. The `/m3u` endpoint and the portal `refreshCredentials` action additionally enforce via `requireUser`, which redirects anon/non-user/inactive sessions to `/`.
