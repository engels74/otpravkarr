...
describe("DispatcharrClient constructor", () => {
  ...
  it("accepts http scheme for loopback hosts", () => {
    expect(() => createClient("http://localhost:8000", "key")).not.toThrow();
    expect(() => createClient("http://127.0.0.1:8000", "key")).not.toThrow();
    expect(() => createClient("http://[::1]:8000", "key")).not.toThrow();
  });
